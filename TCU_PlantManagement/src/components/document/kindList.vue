    ...
    <div class="wrapper">
        ...
        <div id="delete" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" v-if="item">
            <div class="modal-dialog">
                ...
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
                    <h4 class="modal-title" id="myModalLabel">编辑植物类别</h4>
                </div>
                <div class="modal-body" align='center'>
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="panel panel-default">
                                <div class="panel-heading"><h4>修改植物类别</h4></div>
                                <div class="panel-body">
                                    <form class="form-horizontal" role="form">
                                        <div class="form-group">
                                            <label class="col-md-2 control-label">名称</label>
...
export default {
    ...
    methods: {
        ...
        // 初始化数据
        async init(){
            let lists = await apis.kind.findAndCountAll(this.off, this.PageShowSum)
        ...
        async toCreat(){
            ...
            let newItem = await apis.kind.create(this.createItem);
            if(newItem){
                this.init();
        ...
        async toEdit(){
            ...
            let list = await apis.kind.update(this.item);
            if(list){
                this.init();       
                s_alert.basic("修改成功~");
            }
        ...
        async deleteItem(item) {
            if (confirm("确认删除吗？与之关联的数据都会清除喔~")) {
                ...
                let del = await apis.kind.delete(item.id);
                s_alert.basic("删除成功~");
                console.log(del)
                this.init();          
            }
